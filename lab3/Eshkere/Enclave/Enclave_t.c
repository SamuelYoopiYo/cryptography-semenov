...
static sgx_status_t SGX_CDECL sgx_getPD(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_getPD_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_getPD_t* ms = SGX_CAST(ms_getPD_t*, pms);
	ms_getPD_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_getPD_t), ms, sizeof(ms_getPD_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_outbuffer = __in_ms.ms_outbuffer;
	size_t _tmp_len = __in_ms.ms_len;
	size_t _len_outbuffer = _tmp_len;
	char* _in_outbuffer = NULL;

	CHECK_UNIQUE_POINTER(_tmp_outbuffer, _len_outbuffer);
	...
	if (_tmp_outbuffer != NULL && _len_outbuffer != 0) {
		if ( _len_outbuffer % sizeof(*_tmp_outbuffer) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_outbuffer = (char*)malloc(_len_outbuffer)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_outbuffer, 0, _len_outbuffer);
	}
	getPD(_in_outbuffer, _tmp_len, __in_ms.ms_i);
	if (_in_outbuffer) {
		if (memcpy_verw_s(_tmp_outbuffer, _len_outbuffer, _in_outbuffer, _len_outbuffer)) {
...
err:
	if (_in_outbuffer) free(_in_outbuffer);
	return status;
}
